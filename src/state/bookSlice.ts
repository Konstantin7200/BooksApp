...
export const filteredBooksSelector=createSelector(
    [filtersSelector,booksSelector],
    (filtersSelector,booksSelector):Book[]=>{
        return booksSelector.filter((book)=>(book.genre===filtersSelector.genre||filtersSelector.genre===null)&&(book.status===filtersSelector.status||filtersSelector.status===null))
    }
)


export const {addBook,deleteBook,markAsRead,saveBook,setBookToEdit,setFilter}=bookSlice.actions
